...
class ApiClient {
  ...
  // Baseline endpoint context configuration lookup
    ...
    if (!kIsWeb) {
      _dio.httpClientAdapter = IOHttpClientAdapter(
        createHttpClient: () {
          ...
          if (kDebugMode) {
            // Development pipeline bypass authorization
  ...
  Future<Response> postMultipart(
    ...
    // 2. Wrap compiled structure into official Dio FormData context grid
    final FormData formData = FormData.fromMap(formDataMap);

    // 3. Dispatch straight through native authenticated Dio pipeline instance
    return _dio.post(
      path,
      data: formData,
      options: Options(
        headers: {
          'Content-Type': 'multipart/form-data', // Force dynamic payload boundaries override
        },
