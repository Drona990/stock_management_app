...
class _SalesLedgerReportPageState extends State<SalesLedgerReportPage> {
  ...
  Future<void> _fetchAuditData() async {
    ...
    try {
      ...
      print("purchase audit report $response");
      if (response.data != null && response.data['data'] != null) {
        setState(() {
          _cachedAuditData = response.data['data'];
        });
      }
    } catch (e) {
  ...
  // --- PDF GENERATION LOGIC FOR INVOICE REPRINT ---
  ...
  // --- 3. PRINT PDF COMPILING TO EXACT COPIED REPORT UI TABULAR ARCHITECTURE ---
  Future<void> _generateAuditReportPdf(Map<String, List<dynamic>> groupedData) async {
            ...
            build: (pw.Context context) {
              return [
                pw.Header(
                    ...
                    child: pw.Column(
                        ...
                        children: [
                          ...
                          pw.Text("GENERATED STATEMENT DIRECTORY", style: pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
                ...
                ...groupedData.entries.map((entry) {
                  ...
                  double totalCgst = txns.fold(0.0, (sum, i) => sum + parseNum(i['cgst']));
                  double totalSgst = txns.fold(0.0, (sum, i) => sum + parseNum(i['sgst']));
                  double totalIgst = txns.fold(0.0, (sum, i) => sum + parseNum(i['igst']));
                  double grandTotal = txns.fold(0.0, (sum, i) => sum + parseNum(i['grand_total']));
                  ...
                  return pw.Column(
                    ...
                    children: [
                      pw.Padding(
                        ...
                        child: pw.Text("DATE: $date", style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: PdfColors.blueGrey900)),
                      ),
                      pw.Table(
                          ...
                          children: [
                            ...
                            pw.TableRow(
                                ...
                                children: [
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text("PARTY NAME", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text("TAXABLE", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text("CGST", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text("SGST", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text("IGST", style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
                            ...
                            // Data Rows
                            ...txns.map((txn) => pw.TableRow(
                                children: [
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(txn['party_name']?.toString().toUpperCase() ?? '', style: const pw.TextStyle(fontSize: 7.5))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(parseNum(txn['total_taxable']).toStringAsFixed(2), style: const pw.TextStyle(fontSize: 7.5))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(parseNum(txn['cgst']).toStringAsFixed(2), style: const pw.TextStyle(fontSize: 7.5))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(parseNum(txn['sgst']).toStringAsFixed(2), style: const pw.TextStyle(fontSize: 7.5))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(parseNum(txn['igst']).toStringAsFixed(2), style: const pw.TextStyle(fontSize: 7.5))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(parseNum(txn['grand_total']).toStringAsFixed(2), style: const pw.TextStyle(fontSize: 7.5))),
                                ]
                            )),
                            // Total Sub-summary Row
                            pw.TableRow(
                                decoration: const pw.BoxDecoration(color: PdfColors.grey50),
                                children: [
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text("SUBTOTAL", style: pw.TextStyle(fontSize: 7.5, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(totalTaxable.toStringAsFixed(2), style: pw.TextStyle(fontSize: 7.5, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(totalCgst.toStringAsFixed(2), style: pw.TextStyle(fontSize: 7.5, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(totalSgst.toStringAsFixed(2), style: pw.TextStyle(fontSize: 7.5, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(totalIgst.toStringAsFixed(2), style: pw.TextStyle(fontSize: 7.5, fontWeight: pw.FontWeight.bold))),
                                  pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(grandTotal.toStringAsFixed(2), style: pw.TextStyle(fontSize: 7.5, fontWeight: pw.FontWeight.bold))),
                                ]
                            )
                          ]
                      ),
    ...
    await Printing.layoutPdf(onLayout: (format) async => pdf.save(), name: 'Audit_Report_${DateTime.now().millisecondsSinceEpoch}.pdf');
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Build matching grouped structured data mappings inside higher context frame levels for printing
    ...
    for (var txn in filteredAuditList) {
      groupedMap.putIfAbsent(txn['txn_date']?.toString() ?? 'N/A', () => []).add(txn);
    }
    ...
    return BlocProvider(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          // --- 1. SEPARATE INDUSTRIAL PROFESSIONAL TOP RIGHT CORNER NAVIGATION BUTTONS ---
          actions: [
            Padding(
            ...
            if (_isAuditMode)
              IconButton(
                ...
                tooltip: "Print Complete Report UI",
                onPressed: () => _generateAuditReportPdf(groupedMap),
              ),
        ...
        body: Column(
          children: [
            ...
            Expanded(
              child: _isAuditMode
                  ? _buildAuditReportList(filteredAuditList, groupedMap)
  ...
  // --- 2. INTEGRATED PRE-FILTERED SEARCH ENGINE CAPABILITY INTERFACE ---
  Widget _buildAuditReportList(List<dynamic> filteredList, Map<String, List<dynamic>> grouped) {
    ...
    return Column(
      children: [
        Padding(
          padding: EdgeInsetsGeometry.fromSTEB(1,1,1,1),
          child: Row(
            children: const [
                  ...
                  ...txns.map((txn) => Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(child: Text(parseNum(txn['cgst']).toStringAsFixed(2), style: const TextStyle(fontSize: 10, color: Color(0xFF334155)))),
                        Expanded(child: Text(parseNum(txn['sgst']).toStringAsFixed(2), style: const TextStyle(fontSize: 10, color: Color(0xFF334155)))),
                        Expanded(child: Text(parseNum(txn['igst']).toStringAsFixed(2), style: const TextStyle(fontSize: 10, color: Color(0xFF334155)))),
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(child: Text(grandTotal.toStringAsFixed(2), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 10, color: Color(0xFF16A34A)))),
                        ],
                      ),
                    ),
                  ),
...
class _SalesLedgerReportPageState extends State<SalesLedgerReportPage> {
  ...
  Future<void> _fetchAuditData() async {
    ...
    try {
      ...
      if (response.data != null && response.data['data'] != null) {
        setState(() {
          // Strict filtering out any non-sales entries directly at the ingestion boundary
        ...
        pw.MultiPage(
            ...
            build: (pw.Context context) {
              return [
                ...
                // --- GRAND TOTAL ACCUMULATION ROW ARCHITECTURE ---
                pw.SizedBox(height: 10),
                pw.Container(border: pw.Border.all(color: PdfColors.black, width: 1), color: PdfColors.grey200),
                pw.Table(
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: ListView.builder(
            padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
            itemCount: grouped.length + 1, // Added 1 extra row index allocations space matching grand layouts summary metrics
            itemBuilder: (context, index) {
              if (index == grouped.length) {
                // --- SCREENSHOT COMPLIANT GRAND BOTTOM CALCULATION ROW ---
                return Padding(
              ...
              return Column(
                ...
                children: [
                  ...
                  Padding(
                    padding: const EdgeInsets.only(top: 6.0, bottom: 16.0),
                    child: Container(
                      decoration: const Border(bottom: BorderSide(color: Color(0xFFE2E8F0)), top: BorderSide(color: Color(0xFFE2E8F0))),
                      padding: const EdgeInsets.symmetric(vertical: 4),
                      child: Row(
                        children: [
                          const Expanded(flex: 3, child: SizedBox()),
  ...
  // --- 4. MODIFIED COMPACT PROFESSIONAL BUDGETED DIALOG BOX DATE RANGE PICKER ---
  Widget _dateFilterButton() {
    return InkWell(
      onTap: () async {
