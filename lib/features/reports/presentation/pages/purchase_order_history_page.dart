...
class _PurchaseOrderHistoryPageState extends State<PurchaseOrderHistoryPage> {
  ...
  // 🟢 PARTY TYPE FILTER STATE: 'ALL', 'SUPPLIER', 'CUSTOMER'
  ...
  Widget _buildHistoryCardsList(bool isMobile) {
    return BlocBuilder<PurchaseOrderBloc, PurchaseOrderState>(
      builder: (context, state) {
        if (state is PoCounterLoaded) return const Center(child: CircularProgressIndicator(color: Color(0xFF00BCD4), strokeWidth: 1.5));
        if (state is PoHistoryLoadedState) {
          final rawList = state.poList;

          // 🟢 FILTER LIST BASED ON SELECTED SWITCH ITEM ('ALL', 'SUPPLIER', 'CUSTOMER')
          final list = rawList.where((item) {
            ...
            // Fallback checking if party_type is null in legacy backend database records
          ...
          if (list.isEmpty) {
            return Center(
              child: Padding(
                ...
                child: Text(
                  "Zero operational purchase orders found for filter '$_selectedPartyFilter'.",
  ...
  // Dynamic Status Action Controller Dropdown Button
  Widget _buildStatusDropdownButton(BuildContext context, dynamic poId, String currentStatus, Color statusColor) {
    return Container(
      height: 24,
      padding: const EdgeInsets.symmetric(horizontal: 4),
