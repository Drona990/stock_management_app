    ...
    on<ConnectRealtimeStream>((event, emit) async {
      try {
        ...
        if (token == null || token.isEmpty) {
          emit(SessionClusterError("Authentication token missing. Re-login required."));
          return;
        }

        // 🌟 DYNAMIC DOMAIN RESOLUTION (NO LOCAL IP HARDCODING)
        ...
        // Wapas purane explicit session counter matrix path par target kiya
        if (baseHttpUrl.startsWith("https://")) {
          final String cleanDomain = baseHttpUrl.replaceFirst("https://", "");
          wsUrlString = "wss://$cleanDomain/ws/realtime-sessions/?token=$token";
        } else if (baseHttpUrl.startsWith("http://")) {
          final String cleanIp = baseHttpUrl.replaceFirst("http://", "");
          wsUrlString = "ws://$cleanIp/ws/realtime-sessions/?token=$token";
        } else {
        ...
        _wsSubscription = _wsChannel!.stream.listen((message) {
          ...
          final List dynamicData = decoded['data'] ?? decoded['payload'] ?? decoded['active_sessions'] ?? [];
        ...
        emit(SessionClusterError("WebSocket Realtime Telemetry Cluster unreachable."));
      }
    });
    ...
    on<TerminateAllUserSessions>((event, emit) async {
      try {
        if (state is SessionClusterLiveLoaded) {
          final currentList = (state as SessionClusterLiveLoaded).userSessionTelemetry;
          ...
          final updatedList = currentList.map((user) {
            if (user['user_id'].toString() == event.userId) {
              // Ek naya map banao copy karke taaki reference change ho sake
              final modifiedUser = Map<String, dynamic>.from(user);
              modifiedUser['is_online'] = false;
              modifiedUser['is_active'] = false;
              modifiedUser['devices'] = []; // Devices array saaf kar diya!
              return modifiedUser;
            }
            return user;
          }).toList();

          // Emit with fresh new collection instance
        ...
        debugPrint("Failed to dispatch global purge sequence command block: $e");
      }
    });
