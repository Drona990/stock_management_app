...
import 'package:stock_management/features/transaction/presentation/pages/quotation_pdf_generator.dart';
...
import '../../../reports/presentation/pages/quotation_history_page.dart';
...
class QuotationRepository {
  ...
  Future<Map<String, dynamic>> saveQuotation(Map<String, dynamic> data) async {
    final response = await apiClient.post('/api/transactions/quotation/', data: data);
    return response.data as Map<String, dynamic>;
  }

  Future<int> fetchNextQuotationCounter() async {
    try {
      final response = await apiClient.get('/api/transactions/quotation/next_quotation_number/');
      if (response.statusCode == 200 && response.data != null) {
        return int.tryParse(response.data['next_quotation_bill_no'].toString()) ?? 1;
      }
      return 1;
    } catch (_) {
      return 1;
  ...
  Future<List<dynamic>> fetchQuotationHistoryRegistry({String? search}) async {
    try {
      final response = await apiClient.get('/api/transactions/quotation/', query: {
        if (search != null && search.isNotEmpty) 'search': search,
      });

      print("quotation history $response");
  ...
  Future<void> updateQuotationStatus(dynamic id, String status) async {
    try {
      await apiClient.post(
        '/api/transactions/quotation/$id/update-status/',
        data: {'status': status},
      );
    } catch (e) {
      throw Exception("Failed to sync quotation status update over server: $e");
    }
  }
}
...
class PreloadNextQuotationNumber extends QuotationEvent {}
class CommitQuotationSave extends QuotationEvent { final Map<String, dynamic> payload; CommitQuotationSave(this.payload); }
class LoadQuotationHistoryEvent extends QuotationEvent { final String? query; LoadQuotationHistoryEvent({this.query}); }
class UpdateQuotationStatusEvent extends QuotationEvent {
  final dynamic quotationId;
  final String status;
  UpdateQuotationStatusEvent({required this.quotationId, required this.status});
}

abstract class QuotationState {}
class QuotationInitial extends QuotationState {}
class QuotationLoading extends QuotationState {}
class QuotationPreloadingCounter extends QuotationState {}
class QuotationSavingInProgress extends QuotationState {}
class QuotationCounterLoaded extends QuotationState { final int nextQuotationNo; QuotationCounterLoaded(this.nextQuotationNo); }
class QuotationSaveSuccess extends QuotationState { final Map<String, dynamic> resData; QuotationSaveSuccess(this.resData); }
class QuotationHistoryLoadedState extends QuotationState { final List<dynamic> quotationList; QuotationHistoryLoadedState(this.quotationList); }
class QuotationFailure extends QuotationState { final String errorMsg; QuotationFailure(this.errorMsg); }
class QuotationStatusUpdateSuccess extends QuotationState {}

class QuotationBloc extends Bloc<QuotationEvent, QuotationState> {
  final QuotationRepository repo;

  QuotationBloc(this.repo) : super(QuotationInitial()) {
    on<PreloadNextQuotationNumber>((event, emit) async {
      emit(QuotationPreloadingCounter());
      try {
        final counter = await repo.fetchNextQuotationCounter();
        emit(QuotationCounterLoaded(counter));
      } catch (e) {
        emit(QuotationFailure(e.toString()));
      }
    });

    on<CommitQuotationSave>((event, emit) async {
      emit(QuotationSavingInProgress());
      try {
        final data = await repo.saveQuotation(event.payload);
        emit(QuotationSaveSuccess(data));
      } catch (e) {
        emit(QuotationFailure(e.toString()));
      }
    });

    on<LoadQuotationHistoryEvent>((event, emit) async {
      emit(QuotationLoading());
      try {
        final history = await repo.fetchQuotationHistoryRegistry(search: event.query);
        emit(QuotationHistoryLoadedState(history));
      } catch (e) {
        emit(QuotationFailure(e.toString()));
      }
    });

    on<UpdateQuotationStatusEvent>((event, emit) async {
      emit(QuotationLoading());
      try {
        await repo.updateQuotationStatus(event.quotationId, event.status);
        emit(QuotationStatusUpdateSuccess());

        final freshHistory = await repo.fetchQuotationHistoryRegistry();
        emit(QuotationHistoryLoadedState(freshHistory));
      } catch (e) {
        emit(QuotationFailure(e.toString()));
      }
    });
...
class QuotationRowController {
  ...
  QuotationRowController({required this.sno});
...
// =============================================================================
class QuotationTerminalScreen extends StatelessWidget {
  const QuotationTerminalScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider<QuotationBloc>(
      create: (context) => QuotationBloc(sl<QuotationRepository>())..add(PreloadNextQuotationNumber()),
      child: const _QuotationTerminalFormView(),
    );
  }
}

class _QuotationTerminalFormView extends StatefulWidget {
  const _QuotationTerminalFormView();

  @override
  State<_QuotationTerminalFormView> createState() => _QuotationTerminalFormViewState();
}

class _QuotationTerminalFormViewState extends State<_QuotationTerminalFormView> {
  final _quotationNoCtrl = TextEditingController();
  final _quotationDateCtrl = TextEditingController(text: DateFormat('dd-MM-yyyy').format(DateTime.now()));
  ...
  // Default set to CUSTOMER for Quotations
  bool _isSupplier = false;
  int? _selectedPartyId;

  List<QuotationRowController> rows = [QuotationRowController(sno: 1)];
  ...
  @override
  void dispose() {
    _quotationNoCtrl.dispose(); _quotationDateCtrl.dispose(); _dueDaysCtrl.dispose(); _pkgCtrl.dispose();
  ...
  void _resetForm() {
    setState(() {
      _quotationDateCtrl.text = DateFormat('dd-MM-yyyy').format(DateTime.now());
      ...
      rows = [QuotationRowController(sno: 1)];
      totalPcs = 0; taxableAmt = 0; totalTax = 0; grandTotal = 0; roundOff = 0;
    });
    context.read<QuotationBloc>().add(PreloadNextQuotationNumber());
  }
  ...
  void _onCommitSave() {
    ...
    final payload = {
      "billdate": DateFormat('yyyy-MM-dd').format(DateFormat('dd-MM-yyyy').parse(_quotationDateCtrl.text)),
    ...
    context.read<QuotationBloc>().add(CommitQuotationSave(payload));
  }

  Future<void> _triggerInstantPrint(Map<String, dynamic> generatedQuotation) async {
    try {
      ...
      final pdfDoc = await InvoiceQuotationPdfService.generate(
        logoImage: logoImage,
        data: generatedQuotation,
        terminalMode: "QUOTATION",
        headings: ["ORIGINAL CLIENT COPY", "OFFICE DUPLICATE REF", "ACCOUNTS ARCHIVE"],
      );

      await Printing.layoutPdf(
        onLayout: (format) async => pdfDoc.save(),
        name: 'QT_${generatedQuotation['quotation_bill_no'] ?? generatedQuotation['billno'] ?? 'VOUCHER'}.pdf',
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    const Color quotationThemeColor = Color(0xFF0F4C81);

    return BlocListener<QuotationBloc, QuotationState>(
      listener: (context, state) {
        if (state is QuotationSavingInProgress) {
          ...
          if (state is QuotationSaveSuccess || state is QuotationFailure) {
            Navigator.of(context, rootNavigator: true).pop();
          }
          if (state is QuotationCounterLoaded) {
            setState(() => _quotationNoCtrl.text = state.nextQuotationNo.toString());
          }
          if (state is QuotationSaveSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Quotation Voucher Committed Successfully!"), backgroundColor: Colors.green));
            _triggerInstantPrint(state.resData).then((_) {
              if (mounted) _resetForm();
            });
          } else if (state is QuotationFailure) {
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: quotationThemeColor,
          toolbarHeight: isMobile ? 120 : 72,
          title: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text("QUOTATION PLACEMENT ENGINE", style: TextStyle(fontSize: 12, color: Colors.cyanAccent, fontWeight: FontWeight.bold)),
            ...
            ElevatedButton.icon(
              ...
              label: const Text("VIEW QUOTATION HISTORY", style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900)),
              ...
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const QuotationHistoryPage())),
            ),
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const Text("ESTIMATED OFFER VALUE", style: TextStyle(fontSize: 7.5, color: Colors.white60, fontWeight: FontWeight.bold)),
            ]),
          ]),
        ),
        body: SingleChildScrollView(
          ...
          child: Column(children: [
            isMobile
                ? Column(children: [_section("SECTION 1: QUOTATION METADATA", _buildInvoiceSection()), _section("SECTION 2: PARTY ALLOCATION", _buildMasterSection())])
                : Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(flex: 5, child: _section("SECTION 1: QUOTATION METADATA", _buildInvoiceSection())),
              const SizedBox(width: 12),
              Expanded(flex: 5, child: _section("SECTION 2: PARTY ALLOCATION", _buildMasterSection())),
            ]),
            _section("SECTION 3: SPECIFICATION MATRIX & PRICING GRID", _buildProductGrid(isMobile)),
            Card(
              elevation: 1, color: const Color(0xFF1F2937),
              child: Padding(padding: const EdgeInsets.all(10), child: Row(children: [
                Expanded(child: _tfGrid(_termsCtrl, "Enter validity clauses, delivery lead times or payment terms...", colorText: Colors.yellowAccent)),
                const SizedBox(width: 16),
                _tfAppBar(_fwdChargeCtrl, "FREIGHT CHARGES")
              ])),
            ),
            const SizedBox(height: 16),
            ElevatedButton.icon(
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0F4C81), foregroundColor: Colors.white, minimumSize: const Size(360, 46), shape: const RoundedRectangleBorder(borderRadius: BorderRadius.zero)),
              onPressed: _onCommitSave,
              icon: const Icon(Icons.assignment_turned_in_rounded, size: 14),
              label: const Text("GENERATE & COMMIT QUOTATION VOUCHER", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
            )
          ]),
        ),
      ),
  ...
  Widget _buildInvoiceSection() => Column(children: [
    Row(children: [
      Expanded(child: _tf(_quotationNoCtrl, "QUOTATION SERIAL NO (AUTO)", readOnly: true)),
      const SizedBox(width: 8),
      Expanded(child: _tfDate(_quotationDateCtrl, "QUOTATION DATE")),
    ]),
    ...
    const SizedBox(height: 10),
    Row(children: [
      Expanded(child: _tf(_dueDaysCtrl, "OFFER VALIDITY TIMELINE (DAYS)", isNum: true)),
      const SizedBox(width: 8),
      Expanded(child: _tf(_pkgCtrl, "EXPECTED CONSIGNMENT PACKAGES")),
    ]),
    const SizedBox(height: 10),
    _tf(_dispatchCtrl, "PROPOSED DISPATCH MODE / TRANSPORT ROUTING"),
  ]);
  ...
  Widget _buildProductGrid(bool isMobile) {
    ...
    return Column(children: [
      Container(padding: const EdgeInsets.all(6), color: const Color(0xFF374151), child: const Row(children: [
        ...
        Expanded(flex: 4, child: Text("ITEM SPECIFICATION PARTICULARS", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
        ...
        Expanded(flex: 1, child: Text("OFFER QTY", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
        Expanded(flex: 1, child: Text("QUOTED RATE", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
  ...
  Widget _itemRowDesktop(int i) {
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(flex: 4, child: _tfGrid(r.productCtrl, "Enter Item Particulars")),
    ...
    return Card(
      ...
      child: Padding(padding: const EdgeInsets.all(10), child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text("QUOTATION LINE ITEM #${i + 1}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 10, color: Colors.blueGrey)),
        ...
        _tf(r.productCtrl, "Item Specification Particulars"),
        const SizedBox(height: 6),
        Row(children: [Expanded(child: _uomDropdown(i)), const SizedBox(width: 6), Expanded(child: _tf(r.hsnCtrl, "HSN CODE"))]),
        const SizedBox(height: 6),
        Row(children: [
          Expanded(child: _tf(r.qtyCtrl, "Quoted Quantity", isNum: true, onCh: (v)=>_calculateTotals())),
          const SizedBox(width: 6),
          Expanded(child: _tf(r.rateCtrl, "Quoted Rate", isNum: true, onCh: (v)=>_calculateTotals())),
        ]),
        const SizedBox(height: 6),
        Row(children: [
        ...
        const SizedBox(height: 6),
        _tf(r.lineTotalCtrl, "Net Line Value", readOnly: true),
      ])),
    );
  }
  ...
  Widget _addLineRowBtn() => Padding(padding: const EdgeInsets.only(top: 4.0), child: ElevatedButton.icon(onPressed: () => setState(() => rows.add(QuotationRowController(sno: rows.length + 1))), style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey.shade700, foregroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.zero)), icon: const Icon(Icons.add, size: 12), label: const Text("ADD NEW QUOTATION ITEM ROW", style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold))));
...
class _QuotationTerminalFormViewState extends State<_QuotationTerminalFormView> {
  ...
  final _refDateCtrl = TextEditingController(text: DateFormat('dd-MM-yyyy').format(DateTime.now()));

  final _salutationCtrl = TextEditingController(text: "Dear Sir,");
  ...
  final _fwdChargeCtrl = TextEditingController(text: "0");
  final _amtInWordsCtrl = TextEditingController(text: "ZERO RUPEES ONLY");
  double totalPcs = 0, taxableAmt = 0, grandTotal = 0;

  bool _isSupplier = false;
  int? _selectedPartyId;

  List<QuotationRowController> rows = [QuotationRowController(sno: 1)];
  List<TextEditingController> termsControllers = [
    TextEditingController(text: "Delivery Period: Within 3 working days from PO confirmation."),
    TextEditingController(text: "50% Advance Payment along with PO confirmation."),
    TextEditingController(text: "50% Balance Payment upon delivery of materials."),
    TextEditingController(text: "Quotation Validity: 15 days from date of issue."),
    TextEditingController(text: "GST: GST will be charged extra as applicable.")
  ];
  ...
  @override
  void dispose() {
    _quotationNoCtrl.dispose(); _quotationDateCtrl.dispose(); _dueDaysCtrl.dispose();
    _refNoCtrl.dispose(); _refNameCtrl.dispose(); _refDateCtrl.dispose();
    _salutationCtrl.dispose(); _subjectCtrl.dispose(); _bodyCtrl.dispose();
    _nameCtrl.dispose(); _addrCtrl.dispose(); _cityCtrl.dispose(); _pinCtrl.dispose();
    _gstNoCtrl.dispose(); _fwdChargeCtrl.dispose(); _amtInWordsCtrl.dispose();
    for (var c in termsControllers) { c.dispose(); }
    super.dispose();
  }
  ...
  void _resetForm() {
    setState(() {
      _quotationDateCtrl.text = DateFormat('dd-MM-yyyy').format(DateTime.now());
      _dueDaysCtrl.text = "0";
      _refNoCtrl.clear(); _refNameCtrl.clear();
      _refDateCtrl.text = DateFormat('dd-MM-yyyy').format(DateTime.now());
      _salutationCtrl.text = "Dear Sir,";
      _subjectCtrl.clear(); _bodyCtrl.clear();
      _clearPartyAddressFields();
      _fwdChargeCtrl.text = "0"; _amtInWordsCtrl.text = "ZERO RUPEES ONLY";
      rows = [QuotationRowController(sno: 1)];
      termsControllers = [TextEditingController()];
      totalPcs = 0; taxableAmt = 0; grandTotal = 0;
    });
    context.read<QuotationBloc>().add(PreloadNextQuotationNumber());
  }
  ...
  void _onCommitSave() {
    if (_selectedPartyId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please select a valid ${_isSupplier ? 'Supplier' : 'Customer'} profile!"),
          backgroundColor: Colors.orange,
        ),
      );
      return;
    ...
    String combinedTerms = termsControllers
        .map((c) => c.text.trim())
    ...
    final payload = {
      ...
      "purchase_order_no": _refNoCtrl.text,
      "reference_name": _refNameCtrl.text,
      "purchase_order_date": DateFormat('yyyy-MM-dd').format(DateFormat('dd-MM-yyyy').parse(_refDateCtrl.text)),

      "salutation": _salutationCtrl.text,
      "subject": _subjectCtrl.text,
      "letter_body": _bodyCtrl.text,

      "party_type": _isSupplier ? "SUPPLIER" : "CUSTOMER",
      "name": _nameCtrl.text, "address": _addrCtrl.text, "city": _cityCtrl.text, "pin": _pinCtrl.text,
      "gst_number": _gstNoCtrl.text,
      "terms_conditions": combinedTerms,

      "total_pcs": totalPcs, "totalamount": taxableAmt,
      "forwading_charge": double.tryParse(_fwdChargeCtrl.text) ?? 0,
      "grand_totamt": grandTotal, "amtin_words": _amtInWordsCtrl.text,
      if (_isSupplier) "supplier": _selectedPartyId else "customer": _selectedPartyId,
      "details": rows.map((r) => {
    ...
    return BlocListener<QuotationBloc, QuotationState>(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text("QUOTATION PLACEMENT ENGINE", style: TextStyle(fontSize: 12, color: Colors.cyanAccent, fontWeight: FontWeight.bold)),
              const SizedBox(height: 4),
              Row(children: [_statLabel("TOTAL QTY", totalPcs.toInt().toString()), _statLabel("NET TOTAL", taxableAmt.toStringAsFixed(2))]),
            ])),
            ElevatedButton.icon(
              icon: const Icon(Icons.history_edu_sharp, size: 12),
              label: const Text("VIEW QUOTATION HISTORY", style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900)),
        ...
        body: SingleChildScrollView(
          ...
          child: Column(children: [
            isMobile
                ? Column(children: [_section("SECTION 1: QUOTATION METADATA & REFERENCES", _buildInvoiceSection()), _section("SECTION 2: PARTY ALLOCATION", _buildMasterSection())])
                : Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(flex: 5, child: _section("SECTION 1: QUOTATION METADATA & REFERENCES", _buildInvoiceSection())),
              const SizedBox(width: 12),
              Expanded(flex: 5, child: _section("SECTION 2: PARTY ALLOCATION", _buildMasterSection())),
            ]),

            _section("SECTION 3: COVER LETTER / APPLICATION WRITING", _buildLetterWritingSection()),
            _section("SECTION 4: SPECIFICATION MATRIX & PRICING GRID", _buildProductGrid(isMobile)),
            _section("SECTION 5: CUSTOM TERMS & CONDITIONS", _buildCustomTermsSection()),
            ...
            Card(
              ...
              child: Padding(
                  ...
                  child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Expanded(child: Text("NET TOTAL IN WORDS: ${_amtInWordsCtrl.text}", style: const TextStyle(fontSize: 10, color: Colors.white70, fontWeight: FontWeight.w500))),
                        _tfAppBar(_fwdChargeCtrl, "FREIGHT CHARGES")
                      ]
                  )
              ),
  ...
  Widget _buildInvoiceSection() => Column(children: [
    Row(children: [
      ...
      Expanded(child: _tfDate(_quotationDateCtrl, "QUOTATION DATE")),
      const SizedBox(width: 8),
      Expanded(child: _tf(_dueDaysCtrl, "VALIDITY (DAYS)", isNum: true)),
    ]),
    const SizedBox(height: 10),
    Row(children: [
      Expanded(child: _tf(_refNoCtrl, "REFERENCE NO")),
      const SizedBox(width: 8),
      Expanded(child: _tf(_refNameCtrl, "REFERENCE NAME / KIND ATTN")),
      const SizedBox(width: 8),
      Expanded(child: _tfDate(_refDateCtrl, "REFERENCE DATE")),
    ]),
  ]);
  ...
  Widget _buildLetterWritingSection() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _tf(_salutationCtrl, "SALUTATION"),
      const SizedBox(height: 10),
      _tf(_subjectCtrl, "QUOTATION SUBJECT"),
      const SizedBox(height: 10),
      TextFormField(
        ...
        decoration: const InputDecoration(
          labelText: "APPLICATION BODY / INTRODUCTORY NOTE",
    ...
    return Column(children: [
      Container(
          ...
          child: const Row(children: [
            ...
            Expanded(flex: 5, child: Text("ITEM SPECIFICATION PARTICULARS", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
            Expanded(flex: 1, child: Text("UOM", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
            Expanded(flex: 1, child: Text("OFFER QTY", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
            Expanded(flex: 2, child: Text("QUOTED RATE", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
            Expanded(flex: 2, child: Text("NET ROW VALUE", style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
            SizedBox(width: 35),
          ])
      ),
      ListView.builder(
  ...
  Widget _itemRowDesktop(int i) {
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(flex: 5, child: _tfGrid(r.productCtrl, "Enter Particulars")),
        Expanded(flex: 1, child: _uomDropdown(i)),
        Expanded(flex: 1, child: _tfGrid(r.qtyCtrl, "0", isNum: true, onCh: (v)=>_calculateTotals())),
        Expanded(flex: 2, child: _tfGrid(r.rateCtrl, "0", isNum: true, onCh: (v)=>_calculateTotals())),
        Expanded(flex: 2, child: _tfGrid(r.lineTotalCtrl, "0.00", readOnly: true)),
        IconButton(
            icon: const Icon(Icons.delete_outline_rounded, color: Colors.red, size: 16),
            onPressed: () => setState(() { if(rows.length > 1) rows.removeAt(i); _calculateTotals(); })
        ),
      ]),
    );
  }
  ...
  Widget _buildMobileProductCard(int i) {
    ...
    return Card(
      margin: const EdgeInsets.only(bottom: 8), color: Colors.grey.shade50,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.zero, side: BorderSide(color: Colors.grey)),
      child: Padding(padding: const EdgeInsets.all(10), child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text("LINE ITEM #${i + 1}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 10, color: Colors.blueGrey)),
          IconButton(icon: const Icon(Icons.delete, color: Colors.red, size: 16), onPressed: () => setState(() { if(rows.length > 1) rows.removeAt(i); _calculateTotals(); })),
        ]),
        _tf(r.productCtrl, "Item Specification"),
        const SizedBox(height: 6),
        Row(children: [
          Expanded(child: _uomDropdown(i)),
          const SizedBox(width: 6),
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text("ENTER TERMS & CONDITIONS POINTS", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
            ElevatedButton.icon(
              onPressed: () => setState(() => termsControllers.add(TextEditingController())),
              style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey.shade800, foregroundColor: Colors.white, elevation: 0),
              icon: const Icon(Icons.add, size: 12),
              label: const Text("ADD POINT", style: TextStyle(fontSize: 9)),
            ),
          ],
        ),
          ...
          return Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(child: _tfGrid(ctrl, "Enter clause point (e.g. 50% Advance with PO...)")),
                IconButton(
