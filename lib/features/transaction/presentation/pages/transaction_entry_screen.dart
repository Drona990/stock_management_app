...
// ==========================================================================
// 1. REPOSITORY LAYER (With Transactional Bulk Upload Endpoint)
// ==========================================================================
class TransactionRepository {
  final ApiClient apiClient = sl<ApiClient>();
  ...
  // 🟢 Dedicated Transactional Bulk Upload (Handles Header + Multi-Details)
  Future<Map<String, dynamic>> uploadBulkInvoices(PlatformFile file, bool isSales) async {
    final endpoint = isSales
        ? '/api/transactions/sales/bulk-upload/'
        : '/api/transactions/purchase/bulk-upload/';
...
// ==========================================================================
// 2. BLOC LAYER (Extended with Bulk Upload Event)
// ==========================================================================
abstract class TransactionEvent {}
...
class BulkUploadInvoiceEvent extends TransactionEvent {
  final PlatformFile file;
  final bool isSales;
  BulkUploadInvoiceEvent(this.file, this.isSales);
}
...
class BulkUploadSuccess extends TransactionState {
  final String message;
  final int invoicesCount;
  final int itemsCount;
  BulkUploadSuccess(this.message, this.invoicesCount, this.itemsCount);
}
class TransactionError extends TransactionState {
  final String message;
  TransactionError(this.message);
...
class TransactionBloc extends Bloc<TransactionEvent, TransactionState> {
  ...
  TransactionBloc(this.repository) : super(TransactionInitial()) {
    ...
    on<BulkUploadInvoiceEvent>((event, emit) async {
      emit(TransactionLoading());
      try {
        final res = await repository.uploadBulkInvoices(event.file, event.isSales);
        if (!isClosed) {
          emit(BulkUploadSuccess(
            res['message'] ?? "Invoices imported successfully!",
            res['invoices_imported'] ?? 0,
            res['items_imported'] ?? 0,
          ));
        }
      } catch (e) {
        if (!isClosed) emit(TransactionError("Bulk Upload Failed: ${e.toString()}"));
      }
    });
  }
}
...
class _TransactionTerminalScreenState extends State<TransactionTerminalScreen> {
  ...
  Future<void> _fetchNextCommercialNumber() async {
    try {
      ...
      // Cache busting query parameter add karein taaki network fresh data fetch kare
      ...
      if (response.statusCode == 200 && response.data != null) {
        final nextNumber = widget.isSales
            ? response.data['next_sales_bill_no'].toString()
            : response.data['next_purchase_bill_no'].toString();

        if (mounted) {
          setState(() {
            _billNoCtrl.text = nextNumber;
          });
        }
      }
    } catch (e) {
      debugPrint("Failed to fetch next sequence number: $e");
    }
  }

  // 🟢 Trigger File Picker for Transactional Bulk Import
  Future<void> _pickAndUploadBulkFile() async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        if (mounted) {
          context.read<TransactionBloc>().add(BulkUploadInvoiceEvent(file, widget.isSales));
        }
      }
    } catch (e) {
  ...
  Future<void> _resetForm() async {
    ...
    await _fetchNextCommercialNumber();
  }
  ...
  Future<void> _generatePdf(Map<String, dynamic> savedData) async {
    try {
      ...
      List<String> copyHeadings = widget.isSales
          ? ["ORIGINAL FOR BUYER", "TRANSPORT COPY", "ACKNOWLEDGEMENT COPY", "EXTRA COPY"]
          : ["PURCHASE VOUCHER"];
  ...
  // 🟢 Updated Build & BlocListener Implementation
  @override
  Widget build(BuildContext context) {
    bool isMobile = MediaQuery.of(context).size.width < 1000;
    final Color terminalThemeColor = widget.isSales ? const Color(0xFF1A252F) : const Color(0xFF2E4053);
    ...
    return BlocListener<TransactionBloc, TransactionState>(
      listener: (context, state) {
        if (state is TransactionLoading) {
          ...
          if (state is TransactionSuccess || state is TransactionError || state is BulkUploadSuccess) {
            Navigator.of(context, rootNavigator: true).pop();
          }
          if (state is TransactionSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text("✅ ${state.message} (${state.invoicesCount} Invoices, ${state.itemsCount} Items)"),
                backgroundColor: Colors.teal,
                duration: const Duration(seconds: 4),
              ),
            );
            // 🟢 Hard sync trigger: Masters aur fresh sequence number reload
            _loadMasters();
            _resetForm();
          } else if (state is TransactionError) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("❌ ${state.message}"), backgroundColor: Colors.red, duration: const Duration(seconds: 5)),
            );
          }
        }
      },
          ...
          actions: [
            Padding(
              ...
              child: OutlinedButton.icon(
                ...
                icon: const Icon(Icons.file_upload_outlined, size: 14, color: Colors.cyanAccent),
                label: Text(
                  widget.isSales ? "IMPORT SALES (XLSX)" : "IMPORT PURCHASE (XLSX)",
