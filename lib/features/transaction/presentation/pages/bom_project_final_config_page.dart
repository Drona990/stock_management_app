...
class ConfigRowNode {
  ...
  ConfigRowNode({
    ...
    this.rawSize = "—",
    this.finishingSize = "—",
    this.processFlow = "LASER CUTTING",
    this.uom = "PCS",
    this.quantity = 1,
...
class _ProjectFinalConfigPageState extends State<ProjectFinalConfigPage> {
        ...
        setState(() {
          ...
          for (var item in itemsList) {
            _configGridRows.add(ConfigRowNode(
              ...
              processFlow: item['process_flow'] ?? "LASER CUTTING",
              uom: item['uom'] ?? "PCS",
  ...
  void _executeManualAutofill(int index, String? code) {
    if (code == null) return;
    final match = _globalMaterialsPool.firstWhere((m) => m['material_code'] == code, orElse: () => {});
    if (match.isEmpty) return;

    setState(() {
      _configGridRows[index].materialId = match['id'];
      _configGridRows[index].materialCode = code;
      _configGridRows[index].description = match['name'] ?? "";
      _configGridRows[index].materialType = match['material_type_detail']?['name'] ?? "—";
      _configGridRows[index].rawSize = match['raw_size'] ?? "—";
      _configGridRows[index].finishingSize = match['finishing_size'] ?? "—";
      _configGridRows[index].uom = match['uom_detail']?['uom_name'] ?? "PCS";
      _configGridRows[index].purchasePrice = double.tryParse(match['purchase_price'].toString()) ?? 0.0;
      _updateSummaryCalculations();
    });
  }
  ...
  // ✅ FIXED & COMPLETED: Core Database Saving Manifest Pipeline
  Future<void> _dispatchFinalManifest() async {
    ...
    final payload = {
      ...
      "delivery_timeline": DateFormat('yyyy-MM-dd').format(DateTime.now()), // Auto capture timeline format
    ...
    try {
      ...
      // ✅ API call targeting our brand new clean backend tables
      ...
      if (res.statusCode == 201 || res.statusCode == 200) {
        ...
        _flushAllTransactionFormFields(); // Clears form cleanly on success
      } else {
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  ElevatedButton.icon(
                    style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0F4C81), foregroundColor: Colors.white, shape: const RoundedRectangleBorder()),
            ...
            Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF115E59), foregroundColor: Colors.white, shape: const RoundedRectangleBorder(), elevation: 0),
                  onPressed: _appendManualRow,
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: const [
                        ...
                        DataColumn(label: Text('MATERIAL CODE *', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Color(0xFF0F4C81)))),
                        ...
                        return DataRow(cells: [
                          ...
                          DataCell(SizedBox(
                            ...
                            child: DropdownButtonFormField<String>(
                              value: _globalMaterialsPool.any((e) => e['material_code'] == rowNode.materialCode) ? rowNode.materialCode : null,
                              hint: const Text("Select", style: TextStyle(fontSize: 10.5)),
                              isDense: true,
                              decoration: const InputDecoration(border: InputBorder.none),

                              // ✅ FIX: .map() ke baad parenthesis check kijiye aur end mein .toList() lagaiye
                              items: _globalMaterialsPool.map<DropdownMenuItem<String>>((m) {
                                return DropdownMenuItem<String>(
                                  value: m['material_code'],
                                  child: Text(m['material_code'] ?? '', style: const TextStyle(fontSize: 10.5)),
                                );
                              }).toList(), // 👈 Ye lagana zaroori hai

                              onChanged: (val) => _executeManualAutofill(idx, val),
                            ),
                          ),),

                          // Editable text cells specifications
                          ...
                          DataCell(SizedBox(
                            width: 130,
                            child: DropdownButtonFormField<String>(
                              value: rowNode.processFlow, style: const TextStyle(fontSize: 10, color: Colors.deepOrange, fontWeight: FontWeight.bold),
                              decoration: const InputDecoration(border: InputBorder.none),
                              items: const [
                                DropdownMenuItem(value: "LASER CUTTING", child: Text("LASER CUTTING")),
                                DropdownMenuItem(value: "MECHANICAL", child: Text("MECHANICAL")),
                                DropdownMenuItem(value: "PNEUMATIC", child: Text("PNEUMATIC")),
                                DropdownMenuItem(value: "EN8 ASSEMBLY", child: Text("EN8 ASSEMBLY")),
                              ],
                              onChanged: (v) { setState(() { rowNode.processFlow = v!; }); },
                            ),
                          )),

                          DataCell(Text(rowNode.uom, style: const TextStyle(fontSize: 10.5))),
                          // 🔍 Dhoondhiye: DataCell(Text("₹ ${rowNode.purchasePrice.toStringAsFixed(2)}", ...))
// 🛠️ REPLACE KAREIN IS CODE SE:
                          ...
                          // Quantity modifier textfield trigger
                          DataCell(SizedBox(
                            width: 50,
                            child: TextFormField(
  ...
  Widget _buildHeaderInput(String label, TextEditingController ctrl, {bool isNum = false}) {
    return Container(
      height: 32,
      decoration: BoxDecoration(color: Colors.white, border: Border.all(color: Colors.grey.shade200)),
