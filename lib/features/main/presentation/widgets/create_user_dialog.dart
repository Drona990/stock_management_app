...
import '../../../../../injection.dart';
import '../pages/bar&resturant/user_management_page.dart';
import '../../../inventory/presentation/bloc/location_bloc.dart'; // ✅ Import Location Bloc
import '../../../inventory/presentation/bloc/location_bloc.dart' as loc; // For clarity
...
class _CreateUserDialogState extends State<CreateUserDialog> {
  ...
  String _selectedRole = "staff";
  int? _selectedLocationId; // ✅ Track selected location ID
  String _currentUserRole = "";
  bool _isLoading = false;
  bool _isPermissionLoaded = false;
  ...
  Future<void> _submitData() async {
    ...
    try {
      ...
      final response = await sl<ApiClient>().post(path, data: {
        ...
        "name": _nameController.text.trim(),
        "role": _selectedRole,
        "location": _selectedLocationId, // ✅ Sending selected location ID to backend
      });
            ...
            child: Column(
              ...
              children: [
                const Text("Provision New Credential",
                    style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF1A1C24))),
                const SizedBox(height: 8),
                const Text("Set up login access and assign a work location.",
                    style: TextStyle(color: Colors.grey, fontSize: 13)),
                const SizedBox(height: 32),
                ...
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _buildLabel("ASSIGN LOCATION"),
                    // ✅ Location Dropdown powered by Bloc
                    BlocBuilder<LocationBloc, LocationState>(
                      builder: (context, state) {
                        List<DropdownMenuItem<int>> locationItems = [];

                        if (state is LocationLoaded) {
                          locationItems = state.locations.map((loc) {
                        ...
                        return DropdownButtonFormField<int>(
                          ...
                          decoration: _inputStyle("", Icons.location_on_outlined),
