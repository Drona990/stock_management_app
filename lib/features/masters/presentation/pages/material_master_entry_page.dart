...
class MaterialEntity {
  ...
  factory MaterialEntity.fromJson(Map<String, dynamic> json) => MaterialEntity(
    ...
    imagePath: json['image'], // Complete image absolute or relational API endpoint string
  );
...
class MatInit extends MatState {}
...
class MaterialMasterBloc extends Bloc<MatEvent, MatState> {
  ...
  MaterialMasterBloc(this.repo) : super(MatInit()) {
    ...
    on<AddMat>((e, emit) async {
      try {
        ...
        if (e.onSuccess != null) e.onSuccess!(); // Trigger explicit clearing sequence calls
        add(LoadMats());
      } catch (_) {}
    });
...
// ==========================================================================
// 4. MAIN HIGH-DENSITY VISUAL CONTROLLER CANVAS
// ==========================================================================
...
class _MaterialMasterPageState extends State<MaterialMasterPage> {
  ...
  Future<void> _loadDropdownMastersPool() async {
    try {
      ...
      setState(() { _uomsList = uoms; _typesList = types; _isInitialPoolLoading = false; });
      ...
      child: Builder(
        builder: (newContext) {
          ...
          return Scaffold(
            ...
            endDrawer: Drawer(
              width: isMobileLayout ? screenWidth * 0.85 : 460,
              shape: const RoundedRectangleBorder(),
              child: _buildEnhancedSideFormPanel(newContext, bloc),
            ),
            body: Container(
              ...
              child: Column(
                ...
                children: [
                  const Text("PRODUCT MASTER REGISTRIES", style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Color(0xFF1E293B))),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Container(
                          ...
                          child: TextField(
                            ...
                            onChanged: (val) => bloc.add(LoadMats(query: val.trim())),
                            decoration: const InputDecoration(
                              hintText: "Filter registries by item properties...",
                              prefixIcon: Icon(Icons.search_rounded, size: 13, color: Colors.blueGrey),
                              border: InputBorder.none, contentPadding: EdgeInsets.only(bottom: 14),
                            ),
                          ),
                        ),
                      ),
                      ...
                      ElevatedButton.icon(
                        ...
                        icon: const Icon(Icons.add_box_outlined, size: 12),
                        label: const Text("NEW REGISTRY ITEM", style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                  const SizedBox(height: 12),
                  ...
                  Expanded(
                    child: BlocBuilder<MaterialMasterBloc, MatState>(
                      builder: (context, state) {
                        if (state is MatLoading) return const Center(child: CircularProgressIndicator(strokeWidth: 1.5));
                        if (state is MatError) return Center(child: Text(state.msg, style: const TextStyle(color: Colors.red, fontSize: 11)));
                        if (state is MatLoaded) {
                          if (state.items.isEmpty) return const Center(child: Text("Zero registries match.", style: TextStyle(fontSize: 11)));

                          return isMobileLayout ? _buildMobileCardView(state.items, bloc) : _buildDesktopGridTable(state.items, bloc);
                        }
  ...
  // ==========================================================================
  // 5. SPREADSHEET LAYOUT DISPLAY WITH ACTIVE INLINE IMAGES
  // ==========================================================================
  Widget _buildDesktopGridTable(List<MaterialEntity> list, MaterialMasterBloc bloc) {
      ...
      child: Column(
        children: [
          Container(
            ...
            child: const Row(
              children: [
                Expanded(flex: 8, child: Text("PREVIEW", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))), // 🔥 Added row image column layout
                Expanded(flex: 10, child: Text("DATE", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 15, child: Text("MATERIAL CODE", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 22, child: Text("MATERIAL NAME", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 10, child: Text("UOM", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 8, child: Text("UNITS", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 15, child: Text("MATERIAL TYPE", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 15, child: Text("RAW / FINISH SIZE", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 10, child: Text("PUR PRICE", textAlign: TextAlign.end, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 10, child: Text("SALES PRICE", textAlign: TextAlign.end, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
                Expanded(flex: 8, child: Text("WIPE", textAlign: TextAlign.end, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 9, color: Colors.blueGrey))),
              ],
            ),
          ),
            ...
            child: ListView.separated(
              ...
              itemBuilder: (ctx, idx) {
                ...
                return Padding(
                  ...
                  child: Row(
                    children: [
                      // 🔥 ACTIVE INLINE IMAGE PREVIEW CELL: Renders absolute URL via Django endpoints
                      Expanded(
                        flex: 8,
                        child: Container(
                          width: 32, height: 32,
                          decoration: BoxDecoration(color: const Color(0xFFF1F5F9), border: Border.all(color: Colors.grey.shade200)),
                          child: _renderInlineGridThumbnail(node.imagePath, serverBase),
                        ),
                      ),
                      Expanded(flex: 10, child: Text(node.entryDate, style: const TextStyle(fontSize: 10))),
                      Expanded(flex: 15, child: Text(node.materialCode, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 10, color: Color(0xFF0F4C81)))),
                      Expanded(flex: 22, child: Text(node.name, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Color(0xFF334155)))),
                      Expanded(flex: 10, child: Text(node.uomName ?? "—", style: const TextStyle(fontSize: 10))),
                      Expanded(flex: 8, child: Text(node.units.toString(), style: const TextStyle(fontSize: 10))),
                      Expanded(flex: 15, child: Text(node.materialTypeName ?? "—", style: const TextStyle(fontSize: 10))),
                      Expanded(flex: 15, child: Text("${node.rawSize ?? '-'} / ${node.finishingSize ?? '-'}", style: const TextStyle(fontSize: 9.5))),
                      Expanded(flex: 10, child: Text(node.purchasePrice.toStringAsFixed(2), style: const TextStyle(fontSize: 10), textAlign: TextAlign.end)),
                      Expanded(flex: 10, child: Text(node.salesPrice.toStringAsFixed(2), style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.green), textAlign: TextAlign.end)),
                      Expanded(flex: 8, child: Align(alignment: Alignment.centerRight, child: IconButton(icon: const Icon(Icons.delete_outline_rounded, size: 13, color: Colors.redAccent), padding: EdgeInsets.zero, constraints: const BoxConstraints(), onPressed: () => bloc.add(DeleteMat(node.id!))))),
                    ],
                  ),
                );
              },
  ...
  Widget _buildMobileCardView(List<MaterialEntity> list, MaterialMasterBloc bloc) {
    final String serverBase = bloc.repo.apiClient.baseUrl;
          ...
          child: Column(
            ...
            children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Text(node.materialCode, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: Color(0xFF0F4C81))),
                            Text(node.entryDate, style: const TextStyle(fontSize: 9, color: Colors.grey)),
                          ],
                        ),
                        const SizedBox(height: 2),
                        Text(node.name, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 11, color: Color(0xFF1E293B))),
                      ],
                    ),
                  )
                ],
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text("Type: ${node.materialTypeName} | Size: ${node.rawSize ?? '-'}", style: const TextStyle(fontSize: 10, color: Colors.black54)),
                  Text("Sales: ₹${node.salesPrice.toStringAsFixed(2)}", style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.green)),
                ],
              ),
              ...
              Align(
                ...
                child: InkWell(
                  onTap: () => bloc.add(DeleteMat(node.id!)),
                  child: const Text("DELETE REGISTRY", style: TextStyle(color: Colors.redAccent, fontSize: 9, fontWeight: FontWeight.bold)),
                ),
              )
            ],
          ),
  ...
  // Helper utility method processing incoming image stream URLs tokens dynamically
  Widget _renderInlineGridThumbnail(String? partialPath, String serverUrl) {
    ...
    // Resolve absolute streaming target address matching routing proxies configuration rules
    final String completeTargetUrl = partialPath.startsWith('http')
        ? partialPath
        : '$serverUrl$partialPath';
  ...
  // ==========================================================================
  // 6. ENHANCED SLIDING PANEL WITH POST-SUBMIT FLUSH FIELDS RESET MATRIX
  // ==========================================================================
  Widget _buildEnhancedSideFormPanel(BuildContext context, MaterialMasterBloc bloc) {
    ...
    return StatefulBuilder(
      builder: (dialogContext, setPanelState) {
        // 🔥 FORM FLUSH CLEANER METHOD: Triggered immediately when transaction hits successfully
        void resetInternalPanelFields() {
          setPanelState(() {
            codeCtrl.clear(); nameCtrl.clear(); rawCtrl.clear();
            finCtrl.clear(); purCtrl.clear(); salesCtrl.clear();
            unitsCtrl.text = "1";
            localPickedImageFile = null;
            webImageMemoryBytes = null;
            selectedUomId = _uomsList.isNotEmpty ? int.tryParse(_uomsList.first['id'].toString()) : null;
            selectedTypeId = _typesList.isNotEmpty ? int.tryParse(_typesList.first['id'].toString()) : null;
          });
        }
        ...
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
        const Text("MATERIAL REGISTER INTERFACE", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 13, color: Color(0xFF1E293B))),
        ...
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
        const Text("LOG ENTRY DATE:", style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
        Text(currentSystemDateString, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: Color(0xFF0F4C81))),
        ],
        ),
        ),
        const SizedBox(height: 16),

        const Text("MATERIAL ASSET IMAGE (OPTIONAL)", style: TextStyle(fontSize: 8.5, fontWeight: FontWeight.bold, color: Colors.blueGrey)),
        const SizedBox(height: 6),
        GestureDetector(
        onTap: () async {
        ...
        setPanelState(() { webImageMemoryBytes = bytes; localPickedImageFile = image.path; });
        } else {
        setPanelState(() { localPickedImageFile = image.path; });
        }
        }
        },
        child: Container(
        ...
        border: Border.all(color: (localPickedImageFile != null || webImageMemoryBytes != null) ? Colors.green.shade400 : Colors.grey.shade300),
        ),
        child: (localPickedImageFile != null || webImageMemoryBytes != null)
        ? Stack(
        children: [
        Positioned.fill(
        child: kIsWeb
        ? Image.memory(webImageMemoryBytes!, fit: BoxFit.cover)
            : Image.file(File(localPickedImageFile!), fit: BoxFit.cover)
        ),
        ...
        SizedBox(width: 6),
        Text("IMAGE SECURED (TAP TO RESET)", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.white)),
        ],
        ))
        ],
        )
        ...
        SizedBox(height: 6),
        Text("TAP TO CHOOSE ATTACHMENT FROM GALLERY POOL", style: TextStyle(fontSize: 8.5, color: Colors.blueGrey, fontWeight: FontWeight.bold)),
        ],
        ),
        ),
        ),
        ...
        Row(children: [Expanded(child: _buildCompactField(codeCtrl, "MATERIAL CODE *")), const SizedBox(width: 10), Expanded(child: _buildCompactField(nameCtrl, "MATERIAL NAME *"))]),
        const SizedBox(height: 12),

        Row(children: [
        Expanded(
        child: DropdownButtonFormField<int>(
        value: (_uomsList.any((element) => int.tryParse(element['id'].toString()) == selectedUomId)) ? selectedUomId : null,
        style: const TextStyle(fontSize: 10, color: Colors.black87),
        decoration: _buildDropdownDecoration("UOM PARAMETER *"),
        ...
        onChanged: (v) => setPanelState(() => selectedUomId = v),
        validator: (v) => v == null ? 'Required' : null,
        ),
        ),
        const SizedBox(width: 10),
        Expanded(child: _buildCompactField(unitsCtrl, "UNITS TOTAL COUNT *", isNumeric: true)),
        ]),
        const SizedBox(height: 12),

        DropdownButtonFormField<int>(
        value: (_typesList.any((element) => int.tryParse(element['id'].toString()) == selectedTypeId)) ? selectedTypeId : null,
        style: const TextStyle(fontSize: 10, color: Colors.black87),
        decoration: _buildDropdownDecoration("MATERIAL TYPE MASTER REFERENCE *"),
        ...
        onChanged: (v) => setPanelState(() => selectedTypeId = v),
        validator: (v) => v == null ? 'Required' : null,
        ),
        const SizedBox(height: 12),

        Row(children: [Expanded(child: _buildCompactField(rawCtrl, "RAW SIZE (e.g. 40X35)")), const SizedBox(width: 10), Expanded(child: _buildCompactField(finCtrl, "FINISHING SIZE (e.g. 35X25)"))]),
        const SizedBox(height: 12),
        Row(children: [Expanded(child: _buildCompactField(purCtrl, "PURCHASE PRICE *", isNumeric: true)), const SizedBox(width: 10), Expanded(child: _buildCompactField(salesCtrl, "SALES PRICE *", isNumeric: true))]),
        ],
        ),
        ),
        ),
        ...
        style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0F172A), foregroundColor: Colors.white, shape: const RoundedRectangleBorder(), elevation: 0),
        onPressed: () {
        if (formKey.currentState!.validate() && selectedUomId != null && selectedTypeId != null) {
        bloc.add(AddMat(
        MaterialEntity(
        entryDate: currentSystemDateString, materialCode: codeCtrl.text.trim(), name: nameCtrl.text.trim(),
        uomId: selectedUomId!, units: int.tryParse(unitsCtrl.text.trim()) ?? 1, materialTypeId: selectedTypeId!,
        rawSize: rawCtrl.text.trim().isEmpty ? null : rawCtrl.text.trim(), finishingSize: finCtrl.text.trim().isEmpty ? null : finCtrl.text.trim(),
        purchasePrice: double.tryParse(purCtrl.text.trim()) ?? 0.0, salesPrice: double.tryParse(salesCtrl.text.trim()) ?? 0.0,
        imagePath: localPickedImageFile,
        webImageBytes: webImageMemoryBytes,
        ),
        // 🔥 ONSUCCESS PIPELINE: Flushes all controllers instantly after backend transaction acknowledges
        onSuccess: () {
        resetInternalPanelFields();
        ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Product Registry Master Sync Complete!"), backgroundColor: Colors.green)
        );
        }
        ));
        }
        },
        child: const Text("SAVE MASTER PRODUCT DATA", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 0.3)),
        ),
        )
        ],
        ),
  ...
  Widget _buildCompactField(TextEditingController ctrl, String label, {bool isNumeric = false}) {
    return TextFormField(
      controller: ctrl, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w500), keyboardType: isNumeric ? TextInputType.number : TextInputType.text,
      decoration: InputDecoration(labelText: label, labelStyle: const TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: Colors.grey), filled: true, fillColor: const Color(0xFFF8FAFC), border: const OutlineInputBorder(borderRadius: BorderRadius.zero), isDense: true, contentPadding: const EdgeInsets.symmetric(vertical: 8, horizontal: 8)),
      validator: (v) => (v == null || v.trim().isEmpty) && label.contains('*') ? 'Mandatory' : null,
    );
  }

  InputDecoration _buildDropdownDecoration(String label) {
    return InputDecoration(labelText: label, labelStyle: const TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: Colors.grey), filled: true, fillColor: const Color(0xFFF8FAFC), border: const OutlineInputBorder(borderRadius: BorderRadius.zero), isDense: true, contentPadding: const EdgeInsets.symmetric(vertical: 6, horizontal: 8));
  }
}*/
